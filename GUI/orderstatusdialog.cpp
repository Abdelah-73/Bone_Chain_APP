...
{
    setWindowTitle(tr("Update Order Status"));
    setFixedSize(250, 150);
    setStyleSheet("background-color: #ffffff; color: #2c3e50; font-size: 14px;");

    QVBoxLayout *layout = new QVBoxLayout(this);
    QLabel *lblInfo = new QLabel(tr("Select the new status:"), this);
    ...
    // 1. Pending (Orange)
    cmbStatus->addItem(tr("Pending"), clsOrder::enStatus::Pending);
    cmbStatus->setItemData(0, QBrush(QColor("#f39c12")), Qt::ForegroundRole);

    // 2. Confirmed (Blue)
    cmbStatus->addItem(tr("Confirmed"), clsOrder::enStatus::Confirmed);
    cmbStatus->setItemData(1, QBrush(QColor("#3498db")), Qt::ForegroundRole);

    // 3. Processing (Purple)
    cmbStatus->addItem(tr("Processing"), clsOrder::enStatus::Processing);
    cmbStatus->setItemData(2, QBrush(QColor("#9b59b6")), Qt::ForegroundRole);

    // 4. Delivered (Green)
    cmbStatus->addItem(tr("Delivered"), clsOrder::enStatus::Delivered);
    cmbStatus->setItemData(3, QBrush(QColor("#27ae60")), Qt::ForegroundRole);

    // 5. Cancelled (Red)
    cmbStatus->addItem(tr("Cancelled"), clsOrder::enStatus::Cancelled);
    ...
    QPushButton *btnSave = new QPushButton(tr("Save Status"), this);
